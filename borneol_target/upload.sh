...
#
# If unspecifed, the destination is boeneol-target.local and the .fw file is naively
# guessed
#
# You may want to add the following to your `~/.ssh/config` to avoid recording
# the IP addresses of the target:
#
# Host boeneol-target.local
...
help() {
    ...
    echo
    echo "Default destination IP is 'boeneol-target.local'"
...
[ -n "$DESTINATION" ] || DESTINATION=boeneol-target.local
