defmodule BorneolTarget do
    ...
    s =
      case VintageNet.get(["interface", "eth0", "addresses"]) do
        ...
        [%{address: my_address} | _] ->
          ...
          my_node = :"borneol-target@#{my_address}"
          host_node = :"borneol-host@#{host_address}"
  ...
  def handle_info(:connect, %{status: :connected, host_node: host_node} = s) do
    ...
    s =
      if Node.connect(host_node) do
          ...
          :timer.sleep(100)
          BorneolApi.set_status(host_node, "Connected. Running tests...")
          run_tests(host_node)
  ...
  defp run_tests(host_node) do
    ...
    BorneolApi.test_results(host_node, results)
  end
